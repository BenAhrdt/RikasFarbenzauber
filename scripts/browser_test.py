...
with tempfile.TemporaryDirectory(prefix='rika-browser-') as tmp:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            expect(page.locator('#canvas [data-stroke]')).to_have_attribute('fill','#edc56b')
            page.locator('#clear-drawing').click()
            page.get_by_role('button',name='╱ Linie',exact=True).click()
            page.locator('#canvas').scroll_into_view_if_needed()
            bounds=page.locator('#canvas').bounding_box()
            lines=[(.20,.22,.75,.18),(.22,.65,.72,.53),(.30,.10,.45,.75),(.58,.08,.68,.72)]
            for ax,ay,bx,by in lines:
                session.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':bounds['x']+bounds['width']*ax,'y':bounds['y']+bounds['height']*ay}]})
                session.send('Input.dispatchTouchEvent',{'type':'touchMove','touchPoints':[{'x':bounds['x']+bounds['width']*bx,'y':bounds['y']+bounds['height']*by}]})
                session.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]})
            expect(page.locator('#canvas [data-stroke]')).to_have_count(4)
            page.get_by_role('button',name='🪣 Füllen',exact=True).click()
            page.locator('#canvas').tap(position={'x':bounds['width']*.50,'y':bounds['height']*.38})
            expect(page.locator('#canvas [data-stroke]')).to_have_count(5)
            expect(page.locator('#canvas [data-stroke][fill="#edc56b"]')).to_have_count(1)
            page.locator('#save').click()
            expect(page.locator('#status')).to_contain_text('Gespeichert!')
            page.reload()
            expect(page.locator('#canvas [data-stroke]')).to_have_count(5)
